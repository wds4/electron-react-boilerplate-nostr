...
const Profile = ({pubkey}) => {
  const { data: userrData } = useProfile({
    pubkey,
  });
  ...
  return (
    <>
      <p>pk: {pubkey}</p>
      <pre style={{border:"5px dashed purple",margin:"5px",padding:"5px"}} >
      {JSON.stringify(userrData,null,4)}
      </pre>
      <p>Name: {userrData?.name}</p>
      <p>Public key: {userrData?.npub}</p>
      <p>Picture URL: {userrData?.picture}</p>
    </>
  )
}
    ...
    return (
      <>
          <div style={{backgroundColor:"#AFAFAF"}} >
              ...
              <Profile pubkey={window.clickedPubKey} />
          </div>
          {events.map( (event) => {
