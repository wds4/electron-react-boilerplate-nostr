...
export default function RootMessage() {
    ...
    /*
    if ((expandedEventTags.length == 0) || (!expandedEventTags)) {
        return (
            <>
                <div>No thread; show: {expandedEvent.id}</div>
            </>
        )
    }
    */
    ...
    if (1) {
        ...
        if (!root_id) {
            root_id = window.expandedEvent.id;
        }
        root_id = window.expandedEvent.id;
        */
    ...
    return (
        <>
            <div>RootMessage: root_id unfound</div>
        </>
    )
}
