...
export default class App extends React.Component {
    ...
    const forceUpdateMainPage = () => {
        this.forceUpdate();
    }

    async componentDidMount() {
        window.addEventListener('resize', updateMainColWidth);
        var aRelayUrls = await fetchRelays("active")
        console.log("aRelayUrls: "+JSON.stringify(aRelayUrls))
        this.setState({aActiveRelayUrls:aRelayUrls})
        await timeout(1000)
        this.forceUpdateMainPage()
    }
    render() {
        return (
            <NProv relayUrls={this.state.aActiveRelayUrls} />
        );
    }
}
